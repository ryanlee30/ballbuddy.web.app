...
class Details extends Component {
  ...
  componentWillMount = async (e) => {
    ...
    const colRef = db.collection('courts').doc(this.props.match.params.id).collection("waitlist");
  ...
  render() {
    ...
    return(
      <div>
        <Drawer modalElementClass="modal"
          ...
          <div className = "details">
            ...
            <div className="waitlist">
              ...
              {Object.values(this.state.waitlist).map((d) => {
                return(<ul>{d.name} | {d.number} | {d.timeAdded}</ul>);
              })}
            </div>
          </div>
        ) : (
